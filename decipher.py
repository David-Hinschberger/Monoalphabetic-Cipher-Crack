...
def load_trigram_likelihood(path:str) -> Dict[str, float]:
    with open(path) as json_file:
        data = json.load(json_file)
        return data

common_trigrams = load_trigram_likelihood('common_trigrams.json')
...
def rate(text):
    ...
    for window_right_index in range(3, len(text)):
        ...
        if trigramStr in common_trigrams:
            rating += common_trigrams[trigramStr]
    return rating
